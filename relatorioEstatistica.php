						...
						while ($row = T_fetch_array($qry)){
							...
							//---- PARTE DO COMPLETO... ESCREVER OS NOMES DOS TRABALHOS...----//
									...
									while($rowTrabalhos = T_fetch_array($qryTrabalhos)){
										echo "<p><img src='img/doc.gif'>&nbsp;".$rowTrabalhos["Id_Trabalho"]."&nbsp;-&nbsp;".utf8_encode($rowTrabalhos[0]);
										if (hasPerm(8)){
						...
						while ($row = T_fetch_array($qry)){
							...
							if ($ct[$row["Id_Status_Trabalho"]]>0){
								...
								for ($i=0;$i<$ct[$row["Id_Status_Trabalho"]];$i++){
									echo "<p><img src='img/doc.gif'>".utf8_encode($ctt[$row["Id_Status_Trabalho"]][$i])." <i><b>(".utf8_encode($arr_cat[$row["Id_Status_Trabalho"]][$i]).")</b></i>";
						...
						while ($rowRev = T_fetch_array($qryRev)){
							...
							if (T_num_rows($qryTrab)){
								...
								if ($trabCt){
									...
									$i = 0;
									for ($i =0; $i<$trabCt; $i++){
										echo "<p><img src='img/doc.gif'> ".utf8_encode($trabTitle[$i])." <i><b>(".utf8_encode($tipo_trabalho[$i]).")</b></i>";
										if (hasPerm(8)){
