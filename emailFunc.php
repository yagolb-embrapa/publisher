...
define("EMAILCP","cp@cnptia.embrapa.br");
...
function mailThem($trabalho,$tipo,$usr=0){
	...
	$header = "From: Comite de Publicacoes <".EMAILCP.">". "\r\n";
	...
	switch($tipo){
		...
		case 6: 	$trab_str = "\"".$rowTrab["Titulo"]."\" , nr. ".$trabalho.",";
					...
					mail($rowAss["Email_Presidente"],$rowEmail["Tipo"],$corpo,$header,$parameter);
					break;
		case 7: 	$sqlAcomp = "SELECT * FROM ACOMPANHAMENTO WHERE Id_Trabalho = '".$trabalho."' AND Id_Status_Trabalho = 2 ORDER BY Data_Operacao DESC LIMIT 1";
					$qryAcomp = T_query($sqlAcomp);
					$rowAcomp = T_fetch_array($qryAcomp);
